...
**정규화(Normalization)** 는 데이터베이스 테이블을 중복 없이, 논리적으로 깔끔하게 설계하기 위한 규칙·기법이다.
데이터 이상(anomaly)과 중복(redundancy)을 제거하여 데이터 무결성과 조회 성능을 높이는 것이 목적이다.

##### 정규화 단계별 정리

정규형(Normal Form)은 여러 단계가 있지만, 실제 DB 설계에서 가장 많이 쓰이는 것은 1NF → 2NF → 3NF → BCNF 정도다.

###### 1정규형 (1NF)
각 컬럼은 원자값만 가진다(더 이상 쪼갤 수 없는 값, 1개).
...
전화번호가 “2개” 들어있음 → 원자성이 깨짐.

###### 2정규형 (2NF)
부분적 종속성 제거(복합 키의 일부분만 의존하는 컬럼 제거)

**언제 문제 발생?**
테이블의 기본키가 (A, B) 두 개 이상일 때
특정 컬럼이 A 또는 B 하나에만 종속되면 문제.

**해결**
키에 부분적으로 종속된 컬럼들을 분리해서 새로운 테이블로.

###### 3정규형 (3NF)
이행적 종속성(Non-key → Non-key) 제거

즉, 기본키가 아닌 컬럼이 다른 기본키가 아닌 컬럼에 종속될 때 문제.
...
학과장이 “학과”에 종속되므로
학생 테이블이 이 정보를 가지면 안 됨.

###### BCNF (Boyce–Codd Normal Form)
3NF보다 더 엄격하게 “모든 결정자는 후보키여야 한다”는 규칙

교수-강의 관계 같은 복잡한 종속성 해결.
...
단점
- 테이블이 많이 분리됨 → JOIN이 많아짐
- 읽기 성능은 오히려 떨어질 수 있음
→ 그래서 대규모 시스템에서는 “역정규화(denormalization)”도 사용함.
...
### 먼저: 트랜잭션 동시성에서 발생하는 3대 문제
1) **Dirty Read (더티 읽기)**

트랜잭션 A가 아직 커밋하지 않은 임시 데이터를
트랜잭션 B가 읽어버리는 문제.
→ A가 롤백하면 B가 읽은 데이터는 “존재하지 않았던 값”이 됨.

2) **Non-Repeatable Read (비반복 읽기)**

한 트랜잭션이 같은 데이터를 두 번 읽었는데 값이 다름.
**원인:**
트랜잭션 A가 읽은 후, 트랜잭션 B가 값을 수정·커밋해버림.

3) **Phantom Read (팬텀, 유령 읽기)**

같은 조건으로 두 번 조회했는데
데이터가 새로 생기거나 사라짐.
**원인:**
다른 트랜잭션이 INSERT/DELETE를 했기 때문.
...
| 격리 수준                | 설명                  | 문제                  |
| -------------------- | ------------------- | ------------------- |
| **READ UNCOMMITTED** | 커밋 안 된 데이터도 읽음      | Dirty Read, Non-Repeatable Read, Phantom Read          |
...
MVCC에서는 읽기 자체는 절대 막히지 않지만, 특정 경우엔 예외가 있다.

**REPEATABLE READ 이상에서 Gap Lock / Next-Key Lock**
- 다른 트랜잭션이 범위(갭)에 insert 하려 하면 차단될 수 있음
- 하지만 “읽기”가 막히는 건 아님

**SERIALIZABLE**
- 읽기에도 lock이 필요해서 차단될 수 있음.
